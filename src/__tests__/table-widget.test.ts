...
import { parseCellInline, serializeTable, splitRowCells } from "../table-widget";
...
describe("parseCellInline", () => {
  ...
  it("parses plain text as a single text token", () => {
    expect(parseCellInline("plain words")).toEqual([{ type: "text", text: "plain words" }]);
  });
  ...
  it.each(["===triple===", "a == spaced == marker", "== leading-space==", "==trailing-space =="])(
    "leaves invalid highlight delimiters as text: %s",
    (source) => {
  ...
  it("strips backslash escapes so the delimiter renders literally", () => {
    expect(parseCellInline("\\*not bold\\*")).toEqual([{ type: "text", text: "*not bold*" }]);
  });

  it("does not treat an in-word underscore as emphasis", () => {
    expect(parseCellInline("snake_case_var")).toEqual([{ type: "text", text: "snake_case_var" }]);
  });
});
