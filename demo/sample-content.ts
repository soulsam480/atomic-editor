...
function table(rng: () => number, imageless: boolean): string {
  ...
  const lines: string[] = ["| " + header.join(" | ") + " |", "| " + divider.join(" | ") + " |"];
...
function section(
  ...
  for (let i = 0; i < blockCount; i++) {
    ...
    else if (pick < 0.58) parts.push(includeLists ? list(rng) : paragraph(rng));
    else if (pick < 0.7) parts.push(includeLists ? taskList(rng) : paragraph(rng));
    else if (pick < 0.8) parts.push(includeTables ? table(rng, imageless) : paragraph(rng));
    else if (pick < 0.9) parts.push(includeCodeBlocks ? codeBlock(rng) : paragraph(rng));
...
export const SEPARATORS_MODES: SeparatorsMode[] = ["with separators", "no separators"];
...
export const CODE_BLOCKS_MODES: CodeBlocksMode[] = ["with code blocks", "no code blocks"];
...
export function generateSampleMarkdown(size: SampleSize, opts: SampleOptions = {}): string {
  ...
  for (let i = 1; i <= SECTIONS_PER_SIZE[size]; i++) {
    ...
    sections.push(section(rng, i, imageless, includeTables, includeLists, includeCodeBlocks));
  }
